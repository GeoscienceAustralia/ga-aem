...
namespace AEM::INVERTER {
	...
	protected:
		...
		double l1_norm(const Vector& g) const {
			double l1 = 0.0;
		...
		cTrial lambda_search_targetphid(const double& currentlambda, const double& targetphid)
		{
			cTrialCache T;
			T.target = targetphid;
			BracketResult b = brackettarget(T, targetphid, currentlambda);
			cTrial t;
	...
	private:
		...
		BracketResult brackettarget(cTrialCache& T, const double target, const double currentlambda)
		{
			...
			if (T.maxphid() < target) {
				...
				return BracketResult::MINBRACKETED;//min bracketed											
			}
			else return BracketResult::ALLABOVE;//all above target
		}
