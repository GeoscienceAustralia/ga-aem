...
public:
	...
	bool readdatafile()	{		
		...
		if (SubSample == b.ud_size_t())SubSample = 1;
		if (HeaderLines == b.ud_size_t())HeaderLines = 0;
		...
		if (haveconductivity){
			...
			if (cscale == b.ud_double()){
				cscale = 1.0;
			}
		}
		else{
			rscale = b.getdoublevalue("ResistivityScaling");
			if (rscale == b.ud_double()){
				rscale = 1.0;
			}
		}		
