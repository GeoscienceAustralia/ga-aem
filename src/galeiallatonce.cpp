...
namespace AllAtOnce {
	...
	public:
		...
		std::vector<Polygon> IncludePolygons;
		...
		cInputOptions(const cBlock& b) {
			...
			if (b.getvalue("IncludePolygon", pfile)) {
				IncludePolygons.push_back(Polygon(pfile));
			}
	...
	private:
		...
		std::vector<ConductivityLog> ConductivityLogs;
	...
	public:
		...
		void read_conductivity_logs() {
			...
			if (use) {
				...
				for (size_t k = 0; k < flist.size(); k++) {
					ConductivityLog clog(flist[k], true);
					...
					if (InputOp.is_point_included(p)) {
						...
						if (neighbours.size() > 0) {
							clog = ConductivityLog(flist[k], false);
		...
		void B_create_elevation_new() {
			...
			for (size_t bi = 0; bi < ConductivityLogs.size(); bi++) {
				ConductivityLog& clog = ConductivityLogs[bi];
...
{
	using namespace AllAtOnce;
	PetscErrorCode ierr;
	try {
		ierr = PetscInitialize(&argc, &argv, (char*)NULL, (char*)NULL);
