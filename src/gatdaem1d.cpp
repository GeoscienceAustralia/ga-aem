	...
	double* SZ)
{
	...
	std::memcpy(PX, R.primary(XCOMP).data(), sz);
	std::memcpy(PY, R.primary(YCOMP).data(), sz);
	std::memcpy(PZ, R.primary(ZCOMP).data(), sz);
	std::memcpy(SX, R.secondary(XCOMP).data(), sz);
	std::memcpy(SY, R.secondary(YCOMP).data(), sz);
	std::memcpy(SZ, R.secondary(ZCOMP).data(), sz);
}
...
void derivative(void* hS, 
	...
	TDEmResponse<double> R = T.derivative(G,calc);
	...
	double* Response)
{
	...
	size_t sz = sizeof(double) * nw;
	double* p = Response;
	std::memcpy(p, R.primary(XCOMP).data(), sz); p += nw;
	std::memcpy(p, R.secondary(XCOMP).data(), sz); p += nw;
	std::memcpy(p, R.primary(YCOMP).data(), sz); p += nw;
	std::memcpy(p, R.secondary(YCOMP).data(), sz); p += nw;
	std::memcpy(p, R.primary(ZCOMP).data(), sz); p += nw;
	std::memcpy(p, R.secondary(ZCOMP).data(), sz); p += nw;
