...
#make synth data
function noisy_forward(x; nmag=6.0)
	signal = forward(x)
...
function MCMCstep(x,misfit,params)
	...
	accept = false;
	xNew = copy(x);
	#only sample conductivity of the big conductor and its thickness
	k = rand(0:1);
	k = k*(length(x)) + (1-k)*2;
	xNew[k] += params["rSD"][k]*randn();
	# xNew[end] = 100 - xNew[end-1];
...
function runMCMC(nsamples)
	...
	#initialise the chain state by sampling from the prior - we are only varying two parameters,
	#the rest are fixed to be the same as true model
	x = [log10.(true_c);true_t];

	x[2] = params["rmin"][2] + (params["rmax"][2]-params["rmin"][2])*rand();
	x[end] = params["rmin"][end] + (params["rmax"][end]-params["rmin"][end])*rand();
	# x[end] = 100 - x[end-1];
